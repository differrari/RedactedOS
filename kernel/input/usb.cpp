...
bool USBDriver::setup_device(uint8_t address, uint16_t port){
    ...
    usb_device_descriptor* descriptor = (usb_device_descriptor*)kalloc(mem_page, sizeof(usb_device_descriptor), ALIGN_64B, MEM_PRIV_KERNEL, true);
    ...
    usb_string_language_descriptor* lang_desc = (usb_string_language_descriptor*)kalloc(mem_page, sizeof(usb_string_language_descriptor), ALIGN_64B, MEM_PRIV_KERNEL, true);
    ...
    if (use_lang_desc){
        ...
        usb_string_descriptor* prod_name = (usb_string_descriptor*)kalloc(mem_page, sizeof(usb_string_descriptor), ALIGN_64B, MEM_PRIV_KERNEL, true);
        ...
        usb_string_descriptor* man_name = (usb_string_descriptor*)kalloc(mem_page, sizeof(usb_string_descriptor), ALIGN_64B, MEM_PRIV_KERNEL, true);
        ...
        usb_string_descriptor* ser_name = (usb_string_descriptor*)kalloc(mem_page, sizeof(usb_string_descriptor), ALIGN_64B, MEM_PRIV_KERNEL, true);
...
bool USBDriver::get_configuration(uint8_t address){
    ...
    usb_configuration_descriptor* config = (usb_configuration_descriptor*)kalloc(mem_page, sizeof(usb_configuration_descriptor), ALIGN_64B, MEM_PRIV_KERNEL, true);
            ...
            if (dev_type != UNKNOWN){
                ...
                for (uint8_t j = 0; j < hid->bNumDescriptors; j++){
                    if (hid->descriptors[j].bDescriptorType == 0x22){//REPORT HID
                        ...
                        report_descriptor = (uint8_t*)kalloc(mem_page, report_length, ALIGN_64B, MEM_PRIV_KERNEL, true);
