...
void boot_draw_lines(gpu_point current_point, gpu_point next_point, gpu_size size, int num_lines, int separation){
    ...
    for (int j = 0; j < num_lines; j++) {
        ...
        if (current_point.x != 0 && current_point.x < (i32)size.width)
            ccurrent.x += (separation * j);
        if (next_point.x != 0 && next_point.x < (i32)size.width)
            cnext.x += (separation * j);
        if (current_point.y != 0 && current_point.y < (i32)size.height)
            ccurrent.y -= (separation * j);
        if (next_point.y != 0 && next_point.y < (i32)size.height)
