...
void Console::draw_cursor(){
    if (last_drawn_cursor_x >= 0 && last_drawn_cursor_y >= 0){
        ...
        fb_draw_char(dctx, last_drawn_cursor_x * char_width, (last_drawn_cursor_y * line_height)+(line_height/2), line[last_drawn_cursor_x], char_scale, text_color);
    }
...
void Console::redraw(){
    ...
    for (uint32_t y = 0; y < rows; y++){
        ...
        for (uint32_t x = 0; x < columns; x++){
            fb_draw_char(dctx, x * char_width, (y * line_height)+(line_height/2), line[x], char_scale, text_color);
        }
    }
    draw_cursor();
}
