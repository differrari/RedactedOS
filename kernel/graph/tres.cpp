...
void get_window_ctx(draw_ctx* out_ctx){
    ...
    if (node && node->data){
        ...
        *out_ctx = frame->win_ctx;
        ...
        if (p->mm.ttbr0 && size && (p->win_fb_phys != phys || p->win_fb_size != size)) {
            if (p->win_fb_size && p->win_fb_phys) {
                for (uint64_t off = 0; off < p->win_fb_size; off += GRANULE_4KB) mmu_unmap_table((uint64_t*)p->mm.ttbr0, p->win_fb_va + off, p->win_fb_phys + off);
                mm_remove_vma(&p->mm, p->win_fb_va, p->win_fb_va + p->win_fb_size); 
            }

            for (uint64_t off = 0; off < size; off += GRANULE_4KB) mmu_map_4kb((uint64_t*)p->mm.ttbr0, p->win_fb_va + off, (paddr_t)(phys + off), MAIR_IDX_NORMAL, MEM_RW | MEM_NORM, MEM_PRIV_USER);
        ...
        if (p->mm.ttbr0 && size) out_ctx->fb = (uint32_t*)p->win_fb_va;
        frame->pid = p->id;
    }
}
