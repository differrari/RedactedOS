...
extern "C" int32_t socket_bind_udp(socket_handle_t sh, const SockBindSpec* spec, uint16_t port) {
    if (!sh || !spec) return SOCK_ERR_INVAL;
    return reinterpret_cast<UDPSocket*>(sh)->bind(*spec, port);
}

extern "C" int64_t socket_sendto_udp(socket_handle_t sh, uint8_t dst_kind, const void* dst, uint16_t port, const void* buf, uint64_t len) {
...
extern "C" int64_t socket_recvfrom_udp(socket_handle_t sh, void* buf, uint64_t len, net_l4_endpoint* out_src) {
