...
bool USBDriver::setup_device(uint8_t address, uint16_t port){
    ...
    usb_string_language_descriptor* lang_desc = (usb_string_language_descriptor*)kalloc(mem_page, sizeof(usb_string_language_descriptor), ALIGN_64B, MEM_PRIV_KERNEL);
            ...
            if (dev_type != UNKNOWN){
                ...
                for (uint8_t j = 0; j < hid->bNumDescriptors; j++){
                    if (hid->descriptors[j].bDescriptorType == 0x22){//REPORT HID
                        ...
                        report_descriptor = (uint8_t*)kalloc(mem_page, report_length, ALIGN_64B, MEM_PRIV_KERNEL);
