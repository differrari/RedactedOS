...
extern "C" bool network_init(void) {
    dispatch = new NetworkDispatch();
    return dispatch && dispatch->init();
}

extern "C" void network_handle_download_interrupt(void) {
    if (dispatch) dispatch->handle_download_interrupt();
}

extern "C" void network_handle_upload_interrupt(void) {
    if (dispatch) dispatch->handle_upload_interrupt();
}

extern "C" void network_net_task_entry(void) {
...
extern "C" uint16_t network_net_get_pid(void) {
