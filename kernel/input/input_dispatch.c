...
bool register_keypress(keypress kp) {
    ...
    if (!target || target->state == STOPPED || !target->id || !target->pc || !target->sp || (((target->spsr & 0xF) == 0) && !target->mm.ttbr0)) {
...
void sys_set_focus(int pid){
    ...
    if (!target || target->state == STOPPED || !target->id || !target->pc || !target->sp || (((target->spsr & 0xF) == 0) && !target->mm.ttbr0)) return;
...
void sys_unset_focus(bool close){
    ...
    if (npid)
    {
        ...
        if (next && next->focused && next->state != STOPPED && next->id && next->pc && next->sp && ((((next->spsr & 0xF) != 0) || next->mm.ttbr0))) focused_proc = next;
    }
}
