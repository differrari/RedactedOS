...
static void icmp_note_frag_needed(uint8_t ifindex, const uint8_t* raw, uint32_t len) {
    ...
    uint32_t ihl = (uint32_t)(version_ihl & 0x0F) * 4; //https://datatracker.ietf.org/doc/html/rfc1191
...
void icmp_input(uint8_t ifindex, netpkt_t* pkt, uint32_t src_ip, uint32_t dst_ip) {
    ...
    uint32_t pay = len - sizeof(icmp_echo_hdr_t);

    if (type == ICMP_ECHO_REQUEST) {
        uint32_t rlen = (uint32_t)sizeof(icmp_echo_hdr_t) + pay;
        uint8_t* buf = (uint8_t*)zalloc(rlen);
        if (!buf) {
            netpkt_unref(pkt);
            return;
        }
        icmp_echo_hdr_t *rp = (icmp_echo_hdr_t*)buf;
        rp->type = ICMP_ECHO_REPLY;
        rp->code = 0;
        rp->id = bswap16(id);
        rp->seq = bswap16(sq);
        if (pay) memcpy(buf + sizeof(*rp), raw + sizeof(*rp), pay);
        rp->checksum = bswap16(checksum16(buf, rlen));
        ...
        if (l3 && l3->l2) {
            ...
            netpkt_t* reply = netpkt_alloc(rlen, headroom, 0);
            if (reply) {
                void* out = netpkt_put(reply, rlen);
                if (out) {
                    memcpy(out, buf, rlen);
                    ipv4_send_packet(src_ip, PROTO_ICMP, reply, &o, IP_TTL_DEFAULT, 0);
                } else {
                    netpkt_unref(reply);
                }
            }
