...
public:
    explicit HTTPClient(uint16_t pid) : sock(SOCK_ROLE_CLIENT, pid){}
    ~HTTPClient() {sock.close();}

    int32_t connect(SockDstKind kind, const void* dst, uint16_t port) { return sock.connect(kind, dst, port); }
