...
int32_t socket_bind_udp(socket_handle_t sh, const SockBindSpec* spec, uint16_t port);
int64_t socket_sendto_udp(socket_handle_t sh, uint8_t dst_kind, const void* dst, uint16_t port, const void* buf, uint64_t len);
int64_t socket_recvfrom_udp(socket_handle_t sh, void* buf, uint64_t len, net_l4_endpoint* out_src);
